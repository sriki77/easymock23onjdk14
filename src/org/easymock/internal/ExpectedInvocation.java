...
public class ExpectedInvocation {
    ...
    private List createMissingMatchers(Invocation invocation,
            List matchers) {
        if (matchers != null) {
    ...
    private boolean matches(Object[] arguments) {
        ...
        for (int i = 0; i < arguments.length; i++) {
            if (!((IArgumentMatcher) matchers.get(i)).matches(arguments[i])) {
                return false;
            }
        }
        return true;
    ...
    private String myToString() {
        ...
        for (Iterator it = matchers.iterator(); it.hasNext();) {
            ((IArgumentMatcher) it.next()).appendTo(result);
            if (it.hasNext()) {
                result.append(", ");
            }
        }
    ...
    public ExpectedInvocation withMatcher(
    org.easymock.ArgumentsMatcher matcher) {
